...
class CertificateManager {
    ...
    func freshTransferIdentity() -> TransferIdentity? {
        ...
        // NOTE: SecKeychainCreate adds the new keychain to the global search list as a
        // side effect. We deliberately do NOT touch the search list here — the v1.8
        // attempt to filter our keychain back out via SecKeychainSetSearchList correlated
        // with a system-level crash, so we leave the global keychain configuration alone.
        // The cost is some transient pollution of the search list (one entry per active
        // transfer), which is harmless: the keychain holds the same cert content as the
        // app's master keychain and is unlinked when the TransferIdentity is released.
