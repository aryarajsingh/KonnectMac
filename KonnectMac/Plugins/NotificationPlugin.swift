...
class NotificationPlugin: PluginProtocol {
    ...
    func handle(packet: NetworkPacket) {
        ...
        // VoIP call apps (WhatsApp, Telegram, etc.) — their call notifications contain the caller name
        let isVoIPCall = packageName == "com.whatsapp" || packageName == "org.telegram.messenger"
        ...
        // VoIP calls (WhatsApp, Telegram): detect call notifications and pass caller name to telephony
        if isVoIPCall && !isCancel {
            let title = packet.body["title"]?.value as? String ?? ""
            let text = packet.body["text"]?.value as? String ?? ""
            let combined = (title + " " + text).lowercased()

            // Detect active call keywords (incoming/ongoing/ringing)
